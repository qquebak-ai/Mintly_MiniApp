...
async function внутреннийХватит(нужноSol = 0) {
  ...
  if (!внутренний || !внутренний.address) return false;
  /* Остаток неизвестен (узел не ответил) — всё равно идём своим
     кошельком: сервер сам сверит остаток перед подписью. Иначе запуск
     уходил в Phantom, а в Telegram это бесконечное «Деплой…». */
  if (внутренний.sol == null) return true;
  return внутренний.sol >= нужноSol;
}
...
export async function запуститьТокенSol({ имя, тикер, стартовыйВзносSol = 0 }) {
  ...
  /* Есть кошелёк приложения — запускаем им всегда, даже если остатка по
     нашим данным мало: сервер сам сверит его и ответит понятной
     ошибкой. Уход в Phantom из Telegram заканчивался бесконечным
     «Деплой…» — окно ждало кошелёк, которого там нет. */
  const { состояниеВнутреннего } = await import("./appWallet");
  const своё = await состояниеВнутреннего();
  if ((своё && своё.address) || await внутреннийХватит(Number(стартовыйВзносSol || 0) + 0.02)) {
