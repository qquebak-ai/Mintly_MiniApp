...
// лишь у токенов с длинным списком. Держим его ответ полминуты.
const всехКеш = new Map();
const ВСЕХ_МС = 30 * 1000;
...
async function числоДержателей(mint, узлы, минимум = 0) {
  ...
  for (const с of ответ) {
    ...
    if (Number((сумма && сумма.uiAmount) || 0) > минимум) счёт += 1;
  }
  return счёт;
}
...
export async function держатели({ mint }) {
  ...
  if (ненулевых >= 20) {
    ...
    if (было && Date.now() - было.ts < ВСЕХ_МС) {
      держателей = было.сколько != null ? было.сколько : ненулевых;
    } else {
      const сколько = await числоДержателей(mint, свои, (делитель * ПЫЛЬ_ДОЛЯ) / 100);
      всехКеш.set(mint, { ts: Date.now(), сколько });
      if (сколько != null) держателей = сколько;
    }
  }
