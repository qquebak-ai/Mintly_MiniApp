...
/* Параметры кривой. Те же пропорции, что и у кривой на TON: виртуальные
   ...
   Числа решаются из трёх условий разом: порог — 85 SOL, капитализация на
   пороге — 25 тысяч долларов, и та же капитализация в пуле сразу после
   закрытия. Последнее и задаёт всю развесовку: в пул уходят собранные
   85 SOL и весь невыкупленный выпуск, поэтому цена там равна
   85 / (выпуск − проданное). Чтобы она совпала с ценой кривой
   (vSol + собрано)² / (vSol × vTokens), на кривой должно разойтись
   ровно 655 миллионов из миллиарда — отсюда vSol = 94 SOL и
   vTokens = 1,38 миллиарда, а выпуск делится как 660 на продажу и 340
   под пару.

   Расплата за стык — старт повыше: 68 SOL капитализации, около семи
   тысяч долларов. Ниже он быть не может: отношение старта к порогу на
   такой кривой равно (vSol / (vSol + 85))², и старт в 4,7 тысячи
   потребовал бы vSol = 65 — тогда в пуле цена оказалась бы в 1,7 раза
   выше той, по которой только что торговали, и капитализация прыгала бы
   с 25 тысяч до 42 прямо в момент выхода.
...
export const КРИВАЯ = {
  virtualSol: 94 * LAMPORTS,
  virtualTokens: 1_380_000_000 * ЕДИНИЦА,
  tokensForSale: 660_000_000 * ЕДИНИЦА,
  graduationSol: 85 * LAMPORTS,
  /* Комиссии площадки на кривой нет. Она была один процент с каждой
     сделки — деньги небольшие, но на входе и выходе они съедали как раз
     ту разницу, ради которой мемкоин и покупают. Сеть свои копейки
     возьмёт в любом случае, а площадка — нет. У токенов, запущенных
     раньше, комиссия осталась прежней: она записана в их кривую
     навсегда. */
  feeBps: 0,
  ...
  liquidityTokens: 340_000_000 * ЕДИНИЦА,
};
