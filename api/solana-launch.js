...
/* Параметры кривой. Те же пропорции, что и у кривой на TON: виртуальные
   ...
   продаётся 800 миллионов из миллиарда. Капитализация на кривой равна
   (vSol + собрано)² × выпуск / (vSol × vTokens), проданное —
   vTokens × собрано / (vSol + собрано). Отсюда vSol = 65 SOL и
   vTokens = 1.4 миллиарда: на 85 собранных уходит 793 миллиона (запас
   до 800 есть), капитализация выходит 247 SOL — при курсе около 102 $
   это ровно 25 тысяч, а стартует токен с 46 SOL, то есть с 4,7 тысяч.
...
export const КРИВАЯ = {
  virtualSol: 65 * LAMPORTS,
  virtualTokens: 1_400_000_000 * ЕДИНИЦА,
