...
/* Параметры кривой. Те же пропорции, что и у кривой на TON: виртуальные
   ...
   Числа не берутся с потолка, а решаются из трёх условий разом: порог —
   85 SOL, капитализация на пороге — 25 тысяч долларов, на кривой
   продаётся 800 миллионов из миллиарда.

   Капитализация считается по токенам в обороте, то есть по проданным
   кривой: у токена, куда никто не вложил, она нулевая, а дальше растёт
   от самих покупок. В резервах это собрано × (vSol + собрано) / vSol.
   Отсюда vSol = 45 SOL: на 85 собранных выходит 246 SOL, то есть при
   курсе около 102 $ ровно 25 тысяч. vTokens подобран под предел
   продажи: при 1,2 миллиарда на пороге уходит 785 миллионов — влезает в
   800 с запасом.

   Капитализация живёт в SOL, поэтому в долларах ходит вместе с курсом:
   пороговая цифра держится, пока SOL стоит около сотни. */
export const КРИВАЯ = {
  virtualSol: 45 * LAMPORTS,
  virtualTokens: 1_200_000_000 * ЕДИНИЦА,
