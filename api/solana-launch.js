...
/* Параметры кривой. Те же пропорции, что и у кривой на TON: виртуальные
   ...
   Виртуальный SOL задаёт масштаб всей кривой: стартовая капитализация
   равна ему (за вычетом доли под пару), а порог закрытия обязан стоять
   чуть раньше распродажи tokensForSale — при 10 SOL восемьсот миллионов
   расходятся примерно к 29 собранным. Было 30 SOL: токен открывался с
   капитализацией под шесть тысяч долларов, и покупка на один SOL
   двигала её на несчастные шесть процентов. Теперь старт около двух
   тысяч, и та же покупка видна как пятая часть капитализации. */
export const КРИВАЯ = {
  virtualSol: 10 * LAMPORTS,
  virtualTokens: 1_073_000_000 * ЕДИНИЦА,
  tokensForSale: 800_000_000 * ЕДИНИЦА,
  graduationSol: 28 * LAMPORTS,
  feeBps: 100,
