...
/* Параметры кривой. Виртуальные резервы задают стартовую цену и
   ...
   Числа решаются из трёх условий разом: порог — 25 SOL, капитализация
   ...
   25 SOL и весь невыкупленный выпуск, поэтому цена там равна
   25 / (выпуск − проданное). Чтобы она совпала с ценой кривой
   (vSol + собрано)² / (vSol × vTokens), должно выполняться
   vSol = порог² / (капитализация − в пул − порог) = 625 / 200 = 3,125,
   и тогда на кривой расходится 900 миллионов из миллиарда, а 100
   остаются под пару.

   Порог был 85 SOL при vSol = 94, и из этого следовал рост всего в
   3,8 раза: отношение цены на пороге к стартовой равно
   ((vSol + порог) / vSol)². Порог в 25 SOL при той же целевой
   капитализации даёт vSol = 3,125 и рост в 81 раз — старт около трёхсот
   долларов вместо семи тысяч. Платой идёт тонкий пул: в него уходит
   десятая часть выпуска (100 миллионов и 25 SOL) против трети прежде,
   и первая же покупка на 1 SOL забирает четверть выпуска — при таком
   пороге это неизбежно, потому что сам порог всего в двадцать пять
   монет.
...
export const КРИВАЯ = {
  virtualSol: 3.125 * LAMPORTS,
  virtualTokens: 1_012_500_000 * ЕДИНИЦА,
  tokensForSale: 900_000_000 * ЕДИНИЦА,
  graduationSol: 25 * LAMPORTS,
  ...
  liquidityTokens: 100_000_000 * ЕДИНИЦА,
};
