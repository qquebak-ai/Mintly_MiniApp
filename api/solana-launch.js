...
/* Запуск делает шесть дел одной транзакцией — счёт токена, выпуск,
   метаданные, передача права выпуска, кривая и стартовая покупка, —
   поэтому предел вычислений у него свой, повыше. */
const ЕДИНИЦ_НА_ЗАПУСК = 420000;

/* Аренда счёта токена. Величина зависит только от размера счёта и
   меняется раз в эпоху: спрашивать её на каждый запуск — лишний круг к
   узлу перед тем, как человек увидит свой токен. */
let арендаМинтаПамять = { сумма: 0, ts: 0 };
const АРЕНДА_ЖИВЁТ_МС = 10 * 60 * 1000;
async function арендаМинта(connection) {
  if (арендаМинтаПамять.сумма > 0 && Date.now() - арендаМинтаПамять.ts < АРЕНДА_ЖИВЁТ_МС) {
    return арендаМинтаПамять.сумма;
  }
  const сумма = await getMinimumBalanceForRentExemptMint(connection);
  арендаМинтаПамять = { сумма, ts: Date.now() };
  return сумма;
}
...
export async function собратьЗапуск({ wallet, name, symbol, base, buySol }) {
  ...
  /* Аренда и свежий блок — два разных вопроса к узлу, и задавать их по
     очереди значит ждать два круга подряд перед каждым запуском.
     Спрашиваем разом; аренда к тому же меняется раз в эпоху, поэтому
     живёт в памяти и обычно не спрашивается вовсе. */
  const [аренда, блок] = await Promise.all([
    арендаМинта(connection),
    свежийБлок(connection),
  ]);
  const tx = new Transaction();

  /* Место в блоке — за надбавку, как и у сделок. Без неё запуск ждал
     общей очереди: в загруженной сети это лишние секунды между нажатием
     и токеном в сети, а платит за них создатель доли цента. */
  приоритет(tx, ЕДИНИЦ_НА_ЗАПУСК);
...
async function пределПродажи(connection, curve) {
  ...
  if (продано <= 0n) return { предел: 0, кривая: с };

  const k = (vSol + собрано) * (vТокены - продано);
  const порог = vSol + собрано - запас;
  if (порог <= 0n) return { предел: Number(продано), кривая: с };

  const предел = k / порог - (vТокены - продано);
  if (предел <= 0n) return { предел: 0, кривая: с };
  return { предел: Number(предел > продано ? продано : предел), кривая: с };
}

/* Сколько монет вернёт продажа этих единиц — теми же действиями, что и
   программа: выплата по инварианту, минус комиссия площадки.
   Нужно не кривой, а истории: раньше в неё писалась оценка из окна, и
   продажа купленного за 10 SOL значилась ровно десяткой, хотя вернулось
   9,9 — комиссия и движение цены по пути в сумме не попадали. */
function выплатаЗа(кривая, единиц) {
  if (!кривая || !(единиц > 0)) return null;
  const монеты = BigInt(кривая.virtualSol) + BigInt(кривая.realSol);
  const токены = BigInt(кривая.virtualTokens) - BigInt(кривая.tokensSold);
  const сколько = BigInt(Math.floor(единиц));
  if (токены <= 0n || сколько <= 0n) return null;
  const выплата = монеты - (монеты * токены) / (токены + сколько);
  const сбор = (выплата * BigInt(кривая.feeBps || 0)) / 10000n;
  const продавцу = выплата - сбор;
  return продавцу > 0n ? Number(продавцу) / LAMPORTS : 0;
}
...
export async function собратьСделку({ wallet, mint, продажа, amount, minOut }) {
  ...
  if (продажа) {
    ...
    const [остаток, расчёт] = await Promise.all([
    ...
    if (расчёт && расчёт.предел != null && единиц > расчёт.предел) единиц = расчёт.предел;
    if (!(единиц > 0)) throw new Error("продавать нечего");
    выплата = выплатаЗа(расчёт && расчёт.кривая, единиц);
  }
  ...
  return {
    transaction: вBase64(tx),
    curve: curve.toBase58(),
    // Что на самом деле уходит и приходит: количество после подрезки под
    // остаток и запас кривой, и выплата за него. По этим числам
    // приложение пишет историю, а не по оценке из окна.
    tokens: единиц / ЕДИНИЦА,
    solOut: выплата,
  };
}
