...
/* Свежая фраза взамен прежней.
 *
 * Фраза заводится сама при первом обращении к кошельку — то есть ещё до
 * того, как человек нажал «Создать кошелёк», и привязывать его кошелёк к
 * этой случайно возникшей записи неправильно: он её не видел и не
 * выбирал. Поэтому в тот момент, когда он заводит кошелёк осознанно, мы
 * пишем новую и отдаём ему именно её.
 *
 * Звать эту дверь можно только пока копия не сделана: после подтверждения
 * за фразой стоят деньги, и подменять её значит их потерять. */
export async function новаяФраза(db, user, набор) {
  const фраза = bip39.generateMnemonic(256); // 24 слова
  const { error } = await db.from("app_seeds").upsert({
    user_id: user.id,
    seed_enc: зашифровать(Buffer.from(фраза, "utf8"), набор.текущий, user.id),
    key_id: набор.метка,
  }, { onConflict: "user_id" });
