...
async function ветвьSolana(tok, прошлое) {
  try {
    ...
    if (свежая) {
      ...
      /* Пустой ответ узла — это не «сделок не было», это «узел не
         ответил». Затирать им прежнюю историю нельзя: приложение потом
         строит график от пустой кривой, и вся прошлая торговля
         превращается в одну свечу от нуля. */
      if (!сделки.length && прошлое && Array.isArray(прошлое.trades) && прошлое.trades.length) {
        сделки = прошлое.trades.map((п) => ({ time: Number(p_время(п)), дельта: Number(п.d || 0) }));
      }
    }
    ...
    // Цена сутки назад: отматываем собранное на сумму сделок окна, но не
    // считаем в них самую первую сделку кривой — это стартовая покупка
    // создателя любого размера, а не движение рынка, и мерить её тем же
    // метром, что обычную сделку, нельзя: токен младше суток показывал бы
    // огромный процент на собственной раздаче.
    const генезис = сделки[0];
    const сдвиг = заСутки.filter((с) => с !== генезис).reduce((s, с) => s + с.дельта, 0);
...
export default async function handler(req, res) {
  ...
  for (const tok of tokens) {
    ...
    // Если до окна сделок нет (токен младше суток), точка отсчёта —
    // сразу после самой первой сделки кривой, а не нулевой резерв: первая
    // сделка почти всегда стартовая покупка создателя, и любого её
    // размера хватает, чтобы «раздача самому себе» выглядела огромным
    // движением цены за сутки, хотя рынок ещё не открылся.
    const прежняя = цена(доОкна.length ? доОкна[доОкна.length - 1].realTon : (история.length ? история[0].realTon : 0n), params);
