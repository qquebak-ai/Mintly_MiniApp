...
async function страница(сеть, page, список = "trending_pools") {
  ...
  /* Ждём своей очереди долго и из резерва: на этом конце никто не
     смотрит в экран, а витрина без обхода замирает для всех. Две
     секунды, которые стояли здесь раньше, в час пик не выпадали почти
     никогда. */
  const ответ = await gtЗапрос(`${GT}/networks/${сеть}/${список}?page=${page}&include=base_token,dex`, { ждать: 12000, резерв: true });
  if (!ответ.ok) {
