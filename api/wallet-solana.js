...
/* Что оставляем на кошельке при выводе «всё».
 *
 * Одной комиссии мало. Solana не разрешает оставить системный счёт с
 * ненулевым остатком ниже платы за хранение (rent-exempt, 0.00089088
 * SOL): такой перевод сеть отклоняет — и человек видел «сделка не
 * прошла в сети», отправляя весь баланс. Держим сам минимум хранения,
 * комиссию и небольшой запас на приоритетную плату. */
const ХРАНЕНИЕ_СЧЁТА = 0.00089088;
const ЗАПАС_НА_КОМИССИЮ = ХРАНЕНИЕ_СЧЁТА + 0.00051;
...
async function кошелёк(db, user, набор) {
  ...
  if (data) {
    ...
    // Новый кошелёк заводится не тем же самым действием, а следующим:
    // человек должен явно повторить, чтобы наверняка увидеть свежий
    // адрес, а не проскочить мимо разбитого кошелька молча — тем более
    // если это была попытка что-то с него отправить.
    throw new Error("кошелёк повреждён и удалён — повтори, будет создан новый");
  }
...
async function подписатьИОтправить({ db, user, строка, набор, base64, дело, максПеревода, послеОтправки = null }) {
  ...
  // Транзакция ушла — можно показывать её итог, не дожидаясь блока.
  if (послеОтправки) { try { послеОтправки(); } catch { /* не мешаем сделке */ } }
  let готово = false;
  ...
  if (исход === false) {
    ...
    /* Своих логов у простого перевода нет, поэтому причина бывает
       пустой. Тогда говорим то, что знаем: сеть отклонила. */
    const причина = await причинаОтказа(подпись);
    throw Object.assign(new Error(причина || "сеть отклонила перевод"), { код: 400 });
  }
  return подпись;
}
...
async function опросИсхода(подпись, мс, хватит) {
  ...
  while (Date.now() < до) {
    ...
    /* Достаточно processed. Это значит «блок с этой сделкой узел
       обработал»: результат уже известен, и откатывается такой блок
       редко — а ждать confirmed дольше на полсекунды-секунду, и всё это
       время человек смотрит на неизменившийся баланс. */
    if (с.confirmationStatus) return true;
  }
  return null;
}
 ...
 * Раньше звать было некому, и токен, собравший свой порог, продолжал
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (действие === "trade") {
      ...
      /* Три проверки перед деньгами: обещания создателя, замок на время
         полёта прошлой сделки и настоящий остаток токена. Друг от друга
         они не зависят, поэтому идут разом — по очереди это три круга
         (две в базу, одна в сеть) перед каждой сделкой, и человек всё это
         время ждёт. Разбираем ответы в прежнем порядке: важно, какой
         отказ он увидит, а не какой пришёл первым. */
      const [запрет, вПолёте, естьТокена] = await Promise.all([
        запретСделки(db, {
        ...
        сделкаВПолёте(db, user),
        продажа ? балансТокена(строка.address, String(тело.mint)) : Promise.resolve(null),
      ]);
      if (запрет) return res.status(403).json(запрет);
      if (вПолёте) return res.status(409).json({ error: "trade_in_flight" });
      ...
      try {
        ...
        const подпись = await подписатьИОтправить({
          ...
          максПеревода: (продажа ? 0.01 : сумма + 0.01) * LAMPORTS,
          // Итог сделки — всем зрителям графика сразу после отправки.
          послеОтправки: () => {
    ...
    if (действие === "swap") {
      ...
      let собранная = null;
      // Сделка по своей кривой — это не своп биржи, и проверять её нужно
      // правилами сделки: программы кривой в списке свопа нет и быть не
      // должно. Без этого обмен своего токена отбивался стражем
      // («запрещённая программа»), хотя транзакцию собрали мы сами.
      let поКривой = false;
      if (свой) {
